...
**Project:** Kapaki Tatui — Site institucional de loja de celular (SEO local + conversao WhatsApp)
**Researched:** 2026-04-06
**Stack status:** Core stack is decided (Nuxt 3 + Tailwind CSS + TypeScript + Vercel SSR). This research focuses on module selection within that stack.
...
**Atencao:** NAO usar `@nuxtjs/tailwindcss` com Tailwind v4. A integracao correta e via `@tailwindcss/vite` como Vite plugin no `nuxt.config.ts`. O modulo da comunidade tem problemas conhecidos de compatibilidade com v4 (issues abertas em fevereiro 2025).
...
`@nuxtjs/seo` inclui internamente:
...
**Alternativa considerada:** Instalar cada modulo individualmente (robots + sitemap + schema-org). Nao recomendado porque `@nuxtjs/seo` garante versoes compativeis entre si e adiciona `nuxt-site-config` que centraliza a URL do site — critico para sitemap e canonical corretos no Vercel.

**Para LocalBusiness (priority 1 para SEO local):**
O `nuxt-schema-org` (incluido no `@nuxtjs/seo`) tem suporte nativo a `defineLocalBusiness()` e `defineFaqPage()`. Schema e gerado server-side e injetado como JSON-LD estatico em producao — perfeito para SSR no Vercel.
...
**Nao usar:** `@nuxtjs/google-fonts` (modulo da comunidade, mais antigo). O `@nuxt/fonts` e o modulo oficial do time Nuxt e o padrao recomendado para 2025. Ele resolve CLS automaticamente via fallback metrics — o `@nuxtjs/google-fonts` nao tem esse recurso.

**Configuracao no `nuxt.config.ts`:**
```typescript
fonts: {
  families: [
...
```
Isso e suficiente — o modulo detecta o provider Google automaticamente pelo nome da fonte.
...
**Alternativa considerada:** `@nuxtjs/gtm` (Google Tag Manager). Mais pesado, mais complexo. Nao justifica para um site simples com GA4 apenas. `nuxt-gtag` e mais leve e direto.

**Alternativa considerada:** Nuxt Scripts (`@nuxt/scripts`). Tem integracao com Google Analytics como script de terceiro. Valida, mas adiciona mais uma camada de abstração sem beneficio claro para esse caso de uso. `nuxt-gtag` e mais simples.
...
**Aviso Vercel:** O provider Vercel requer listar explicitamente todas as larguras usadas em `<NuxtImg>` no `nuxt.config.ts`. Larguras nao declaradas fazem fallback para a proxima maior. Configurar `image.screens` no config.
...
npx nuxi@latest init kapaki-tatui
...
export default defineNuxtConfig({
  ...
  gtag: {
    id: 'G-XXXXXXXXXX', // placeholder — substituir com ID real
  },
  image: {
  ...
  site: {
    url: 'https://kapaki.com.br', // placeholder — substituir
    name: 'Kapaki Tatui',
